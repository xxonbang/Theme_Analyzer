...
- **사용자 보고**: 5/21 작업 사후 진단에서 발견 — 5/26 모든 main.py cron의 공매도 수집 0건
- **증상**:
  - 5/22 (금) 12:30 Refresh: 82/91 정상
  - 5/25 (월) 12:30 Refresh: 84/96 정상
  - 5/26 (화) 09:05/09:28/12:30 모두 0/82, 0/85, 0/79
- **원인** (KIS API 직접 검증):
  - 5/26 시점 D-1=5/25 호출 → output2 `[{},{},{},...]` (n=39 모두 빈 dict)
  - D-2=5/24, D-3=5/23도 동일 (주말 + 5/25 임시휴장)
  - D-4=5/22 (금) → 정상 응답 (삼성전자 ratio=3.23, 카카오 8.10, 에코프로비엠 5.82)
  - 5/21 fix 시 단순 `today - 1day`만 사용 → 휴장일 누적 시 빈 응답
- **수정** (`main.py:8-2` 단계):
  - D-1부터 D-5까지 순차 시도, 005930 삼성전자로 응답 테스트
  - output2가 비어있지 않은 첫 날짜를 `target_date`로 선택
  - 5일 이내 거래일 없으면 (이례적) 건너뜀
  - 선택된 `target_date`로 79~90 종목 일괄 조회
- **검증 (로컬 KIS API)**:
  - D-1~D-3 (5/25~23) 빈 응답 → D-4 (5/22) 자동 폴백 ✅
  - 005930 ratio=3.23, 035720 ratio=8.10, 247540 ratio=5.82 정상 수신
  - Python AST PASS
- **운영 영향**: 다음 main.py 호출 cron (내일 09:05 Daily Theme Analysis)부터 공매도 데이터 정상 채워짐. short_selling 기준 평가 회복.
