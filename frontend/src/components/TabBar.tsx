...
export function TabBar({ activeTab, onTabChange }: Pick<TabBarProps, "activeTab" | "onTabChange">) {
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-t border-border/50 safe-area-bottom">
        <div className="container px-2 sm:px-4">
          ...
          >
            {/* 슬라이딩 인디케이터 */}
            <div
              className="absolute top-1 bottom-1 rounded-lg bg-primary/10 transition-all duration-200 ease-out"
              style={{
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={cn(
                  "relative z-10 flex items-center justify-center py-2.5 sm:py-3 text-[11px] sm:text-sm font-medium rounded-lg",
...
/** 탭 하위 컨트롤 (인라인 배치용) */
export function TabControls({ activeTab, fluctuationMode, onFluctuationModeChange, compositeMode, onCompositeModeChange }: TabBarProps) {
  ...
  return (
    <div className="bg-card/80 backdrop-blur-sm border-b border-border/50">
      <div className="container px-3 sm:px-4 py-1.5 sm:py-2">
        <div className={cn(showTooltip ? "relative" : "")}>
