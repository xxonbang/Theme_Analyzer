...
### 테마 라이프사이클 참고 (섹터 로테이션 분석 데이터)
- [출현] 단계: 신뢰도 "보통" 이하 주의
- [가속] 단계: 신뢰도 "높음" 가능, 거래대금+수급 동반 확인
- [정점] 단계: 신뢰도 "보통" 이하 주의, 차익실현 리스크 언급
- [쇠퇴] 단계: 재등장 시 새로운 촉매 근거 보강 필요
※ 라이프사이클은 신뢰도 판단 참고용이며, 대장주 선정 시에는 반드시 구체적 종목명과 코드를 명시하세요.
...
- **name은 반드시 실제 한국 상장 종목명(예: "삼성전자", "SK하이닉스")이어야 합니다. "관련 기업", "관련주" 같은 카테고리명 금지.**
- **code는 반드시 6자리 숫자 종목코드(예: "005930")여야 합니다. "N/A" 금지.**
- 구체적 종목을 특정할 수 없는 테마는 해당 테마 자체를 제외하세요.
...
def build_forecast_context(
    ...
    if theme_acc and theme_acc.get("recent_failures"):
        ...
        lines.append("※ 위 테마가 다시 등장 시, 새로운 촉매 근거를 보강하여 신뢰도를 보수적으로 판단하세요.")
